...
from sdf_xarray import download
...
class XRLibrary(Protocol):
    def open_datatree(self, *args, **kwargs) -> xr.DataTree: ...


@pytest.mark.parametrize("xrlib", [xr, sdfxr])
def test_datatree_basic(xrlib: XRLibrary):
    with xrlib.open_datatree(TEST_FILES_DIR / "0000.sdf") as dt:
...
@pytest.mark.parametrize("xrlib", [xr, sdfxr])
def test_datatree_attrs(xrlib: XRLibrary):
    with xrlib.open_datatree(TEST_FILES_DIR / "0000.sdf") as dt:
        assert dt.attrs != {}
...
@pytest.mark.parametrize("xrlib", [xr, sdfxr])
def test_datatree_constant_name_and_units(xrlib: XRLibrary):
    with xrlib.open_datatree(TEST_FILES_DIR / "0000.sdf") as dt:
...
@pytest.mark.parametrize("xrlib", [xr, sdfxr])
def test_datatree_coords(xrlib: XRLibrary):
    with xrlib.open_datatree(TEST_FILES_DIR / "0010.sdf") as dt:
...
@pytest.mark.parametrize("xrlib", [xr, sdfxr])
def test_datatree_particles(xrlib: XRLibrary):
    with xrlib.open_datatree(TEST_FILES_DIR / "0010.sdf", keep_particles=True) as dt:
...
@pytest.mark.parametrize("xrlib", [xr, sdfxr])
def test_datatree_no_particles(xrlib: XRLibrary):
    with xrlib.open_datatree(TEST_FILES_DIR / "0010.sdf", keep_particles=False) as dt:
...
def test_datatree_multiple_files_one_time_dim():
    with sdfxr.open_mfdatatree(TEST_FILES_DIR.glob("*.sdf"), keep_particles=True) as dt:
...
def test_datatree_latex_rename_variables():
    with sdfxr.open_mfdatatree(
        TEST_ARRAYS_DIR.glob("*.sdf"), keep_particles=True
    ) as dt:
...
def test_datatree_open_mfdatatree_data_vars_single():
    with sdfxr.open_mfdatatree(
        TEST_FILES_DIR.glob("*.sdf"), data_vars=["Electric_Field_Ex"]
    ) as dt:
...
def test_datatree_open_mfdatatree_data_vars_sparse_multiple():
    with sdfxr.open_mfdatatree(
        ...
        assert "Particles_Per_Cell" in [
            g.split("/")[-1] for g in dt["Particles"].groups
        ]
...
def test_datatree_erroring_on_mismatched_jobid_files():
    with pytest.raises(ValueError):  # noqa: PT011
        sdfxr.open_mfdatatree(TEST_MISMATCHED_FILES_DIR.glob("*.sdf"))


@pytest.mark.parametrize("xrlib", [xr, sdfxr])
def test_open_datatree_deck_path_default(xrlib: XRLibrary):
    with xrlib.open_datatree(TEST_FILES_DIR / "0000.sdf") as dt:
        assert "deck" in dt.attrs


@pytest.mark.parametrize("xrlib", [xr, sdfxr])
def test_open_datatree_deck_path_failed(xrlib: XRLibrary):
    with pytest.raises(FileNotFoundError):
        xrlib.open_datatree(TEST_FILES_DIR / "0000.sdf", deck_path="non_existent.deck")


@pytest.mark.parametrize("xrlib", [xr, sdfxr])
def test_open_datatree_deck_path_relative(xrlib: XRLibrary):
    with xrlib.open_datatree(TEST_FILES_DIR / "0000.sdf", deck_path="input.deck") as dt:
        assert "deck" in dt.attrs
        assert "constant" in dt.attrs["deck"]


@pytest.mark.parametrize("xrlib", [xr, sdfxr])
def test_open_datatree_deck_path_absolute(xrlib: XRLibrary):
    with xrlib.open_datatree(
...
@pytest.mark.parametrize("xrlib", [xr, sdfxr])
def test_open_datatree_deck_path_absolute_other_path(xrlib: XRLibrary):
    with xrlib.open_datatree(
...
def test_open_mfdatatree_deck_path_default():
    with sdfxr.open_mfdatatree(TEST_FILES_DIR.glob("*.sdf")) as dt:
        assert "deck" in dt.attrs


def test_open_mfdatatree_deck_path_failed():
    with pytest.raises(FileNotFoundError):
        sdfxr.open_mfdatatree(
