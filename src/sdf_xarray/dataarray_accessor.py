...
from .plotting import animate, show, voxel_plot, _recover_vertex_coord
...
class EpochAccessor:
    ...
    def resize(
        ...
        for i in range(len(da_resized.dims)):
            ...
            if coord[-4:] == "_mid":
                ...
                vertex_coord_resized = np.linspace(vertex_coord[0], vertex_coord[-1], new_shape[i]+1)
                # Turn the vertex coord back into a midpoint coord
                da_resized[coord] = ((np.roll(vertex_coord_resized, 1) + vertex_coord_resized)/2)[1:]
            else:
    ...
    def limit(
        ...
        for i in range(len(dims)):
            ...
            # Limit dataarray
            da = da.where(
                da[dims[i]] >= limits[i][0], drop = drop).where(
